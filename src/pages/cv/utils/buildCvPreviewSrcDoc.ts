import { appendHideScrollbarStyleToSrcDocHtml } from './appendHideScrollbarStyleToSrcDocHtml';
...
export function buildCvPreviewSrcDoc(sanitizedHtml: string): string {
  ...
  if (/<\s*html[\s>]/i.test(t)) {
    ...
    const withScroll = appendHideScrollbarStyleToSrcDocHtml(t);
    const withScript = withScroll.replace(/(<\/\s*body\s*>)/i, `${LINK_OPENER_SCRIPT}$1`);
