...
export const getPortfolioMileage = async () => {
  const response = await http.get<PortfolioMileageResponse>(
...
export const putPortfolioMileage = async (body: PutPortfolioMileageItem[]) => {
  const response = await http.put<
    ...
    PortfolioMileageResponse
  >(ENDPOINT.PORTFOLIO_MILEAGE, body);
  return response;
};

/** 활동 요약 - 포트폴리오 마일리지 한 건 추가 설명 수정 (PUT, id는 portfolio 항목 id) */
export const putPortfolioMileageItem = async (
  ...
  >(`${ENDPOINT.PORTFOLIO_MILEAGE}/${id}`, body);
  return response;
};
